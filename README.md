...
- 自定义源界面仅显示自定义 HTTPS 地址和“源目录1”至“源目录5”五级下拉选择。
- 自定义源隐藏系统版本、发行版本号、系统维护与补丁组件、软件仓库和芯片架构选项。
- 选择自定义源时，顶部工作流程同步简化为“产品源 → 源目录选择 → 搜索 → 开始下载 → 下载内容”。

## 1.6.0 更新要点

- 修复切换产品源、目录、组件、软件仓库或芯片架构后，搜索结果仍残留上一仓库软件包的问题。
- 软件仓库调整为单选下拉框，并置于芯片架构之前，使选择顺序与实际仓库 URL 层级一致。
- 搜索结果中的 RPM 下载地址支持双击选择和一键复制。
- 产品源增加“自定义源”，支持填写 HTTPS 镜像根地址，并通过“目录1”至“目录5”逐层选择；发现 `repodata/repomd.xml` 后自动加载仓库。
- 自定义源兼容阿里云 EPEL Archive 等包含站外导航及自身链接的镜像目录，采用按需加载以避免遍历大型历史目录树。
